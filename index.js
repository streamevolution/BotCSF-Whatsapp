...
function calcularRFC(nombre, paterno, materno, fechaStr) {
    ...
    // Simplificación de homoclave para el bot (para evitar cargar toda la tabla de mapeo)
    ...
    let suma = 0;
    for (let i = 0; i < 12; i++) {
        suma += dic.indexOf(rfc12.charAt(i)) * (13 - i);
    }
    let mod = suma % 11;
...
function obtenerALPorEstado(estado) {
    ...
    for (let key in mapaAL) {
        if (estLimpio.includes(key)) return mapaAL[key];
    }
...
client.on('message', async (msg) => {
    ...
    if (texto.length === 18 && !texto.includes(' ') && !texto.includes('\n')) {
        ...
        try {
            ...
            const payloadPdf = {
                nombre: datos.nombre,
                paterno: datos.primerApellido,
                materno: datos.segundoApellido,
                curp: texto,
                rfc: rfcCalculado,
                fechaNac: fechaNacFmt,
                correo: correoFmt,
                estado: datos.entidadNacimiento,
                municipio: muniLimpio,
            ...
            // Usar la convención de guardado "Best_ + Nombre"
